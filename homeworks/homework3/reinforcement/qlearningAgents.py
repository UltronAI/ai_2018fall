...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        return q_values[q_values.argMax()] if len(legal_actions) > 0 else 0
    ...
    def computeActionFromQValues(self, state):
        ...
        for action in legal_actions:
            q_values[action] = self.getQValue(state, action)
        return q_values.argMax() if len(legal_actions) > 0 else None
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        return sum([0] + [weights[f] * features[f] for f in features])
